...
class CardioClient(fl.client.NumPyClient):
    ...
    # ========================================================
    # 🔴 100% LABEL-FLIP ATTACK (0 → 1) AT ROUND 10
    # ========================================================
    def apply_label_flip_attack(self, flip_ratio=1.0):
        ...
        one_idx = torch.where(y_all == 1)[0]
        num_to_flip = int(flip_ratio * len(one_idx))

        if num_to_flip > 0:
            flip_idx = one_idx[torch.randperm(len(one_idx))[:num_to_flip]]
            y_all[flip_idx] = 0  # 🔥 1 → 0

        print(
            f"🔥 ATTACK ACTIVATED | Client {self.client_id} | "
            f"Flip ratio={flip_ratio:.2f} | "
            f"Flipped {num_to_flip}/{len(one_idx)} labels (1 → 0)"
        )
    ...
    def fit(self, parameters, config):
        ...
        # 🔴 ATTACK TRIGGER
        if round_num >= 10:
            self.apply_label_flip_attack(flip_ratio=1.0)
...
if __name__ == "__main__":
    ...
    CLIENT_ID = "C1"
    TRAIN_CSV = "/home/coep/Desktop/HD01_final/HD01_harmful/Server_0.1HD_100%_attack_detect_Mitigate/clients/custom_splits/output_final/3_nodes/client_1.csv"
