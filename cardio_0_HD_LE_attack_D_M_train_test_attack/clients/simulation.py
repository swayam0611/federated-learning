...
class CardioClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Flip labels on-the-fly per batch. 
        # No heavy tensor concatenations in memory loop.
        for X, y in self.trainloader:
            X, y = X.to(DEVICE), y.to(DEVICE)
            
            if self.cid == "0" and self.current_round >= ATTACK_ROUND:
                y = torch.zeros_like(y)  # In-memory fast flip (1 -> 0)
